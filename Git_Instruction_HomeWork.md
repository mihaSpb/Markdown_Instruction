...
- Выбираем параметр `Public` - в таком случае доступ к репозиторию будут иметь все, у кого есть ссылка на него. `Private` - ограничивает доступ к репозиторию.
...
> Команда `git remote -v` показывает связанность с удаленным репозиторием (по умолчанию он указывается, как `origin`, но при связывании это название можно изменить). Ключ `-v` показывает ареса связывания.
