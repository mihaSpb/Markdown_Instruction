...
> Команда `git remote -v` показывает связанност с удаленным репозиторием (по умолчанию он указывается, как `origin`, но при связывании это название можно изменить). Ключ `-v` показывает ареса связывания.
