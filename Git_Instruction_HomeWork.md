...
Да, вы должны быть в состоянии выполнить git reflog --no-abbrevи найти SHA1 для фиксации на конце вашей удаленной ветки, а затем простоgit checkout [sha]. И как только вы закончите эту фиксацию, вы можете просто git checkout -b [branchname]воссоздать ветку оттуда.

Если вы только что удалили ветку, вы увидите что-то вроде этого в своем терминалеDeleted branch <your-branch> (was <sha>). Тогда просто используйте это <sha>в этом однострочнике:
